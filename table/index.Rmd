...
df <- cases %>% 
  ...
  ungroup() %>% 
  select(area_name, new_cases, previous_week, last_week, change, previous_week_rate, 
         last_week_rate, percent_rate)
...
tbl <- reactable(df, 
                 ...
                 columns = list(
                   ...
                   new_cases = colDef(name = "14-day trend", 
                                      ...
                                      sortable = FALSE),
                   previous_week = colDef(header = with_tooltip("Previous week", paste("week ending", format(max(cases$date)-days(12),'%A %d %B'))),
                                          align = "left"),
                   last_week = colDef(header = with_tooltip("Last week", paste("week ending", format(max(cases$date)-days(5),'%A %d %B'))),
                                      align = "left"),
                   change = colDef(name = "Change",
                                   ...
                                   align = "left"),
                   previous_week_rate = colDef(header = with_tooltip("Previous week", paste("week ending", format(max(cases$date)-days(12),'%A %d %B'))),
                                               align = "left"),
                   last_week_rate = colDef(header = with_tooltip("Last week", paste("week ending", format(max(cases$date)-days(5),'%A %d %B'))),
                                           align = "left"),
                   percent_rate = colDef(name = "Change",
                                         style = function(value) {
                                     if (value > 0) {
                                       color <- "#e00000"
                                       } else if (value < 0) {
                 ...
                 columnGroups = list(
                   colGroup(header = "Cases", columns = c("previous_week", "last_week", "change")),
                   colGroup(header = "Per 100,000", columns = c("previous_week_rate", "last_week_rate", "percent_rate"))
                   )
                 )
```
